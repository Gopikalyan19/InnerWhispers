...
// Get the tablet list element
const tabletListElement = document.getElementById('tablet-list-body');

// Get the add tablet form element
const addTabletFormElement = document.getElementById('add-tablet-form');

// Add event listener to the add tablet form
addTabletFormElement.addEventListener('submit', (e) => {
    e.preventDefault();
    const tabletName = document.getElementById('tablet-name').value;
    const dateTime = document.getElementById('date-time').value;
    addTabletToList(tabletName, dateTime);
});

// Function to add tablet to the list
function addTabletToList(tabletName, dateTime) {
    const tabletElement = document.createElement('tr');
    ...
    `;
    tabletListElement.appendChild(tabletElement);
}

// Add event listener to the delete and complete buttons
tabletListElement.addEventListener('click', (e) => {
    if (e.target.classList.contains('delete-btn')) {
        const tabletElement = e.target.parentNode.parentNode;
        tabletElement.remove();
    } else if (e.target.classList.contains('complete-btn')) {
        const tabletElement = e.target.parentNode.parentNode;
        tabletElement.cells[2].textContent = 'Complete';
    }
});
